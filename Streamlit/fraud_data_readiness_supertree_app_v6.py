...
def main():
    ...
    dataset_name = st.sidebar.radio("Training Dataset", options=list(DATASET_FILES.keys()),help="Dataset A is intentionally dirty, Dataset B is severely imbalanced, and Dataset C is cleaner and more balanced. All models can be evaluated on the same balanced test dataset.",)
    ...
    st.header(f"Chosen Datset: {dataset_name}",
    ...
    with conf:
        plot_confusion(test_df[TARGET_COLUMN].astype(int), test_predictions, "Confusion Matrix")
