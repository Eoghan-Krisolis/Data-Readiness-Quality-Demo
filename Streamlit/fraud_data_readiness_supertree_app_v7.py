...
def get_current_page() -> str:
    return st.sidebar.radio("Page", ["Model Explorer", "Predictions"], index=0)
...
def plot_metrics_bar(metrics: dict, title: str = "Metric Scores"):
    metric_df = pd.DataFrame({
        ...
        "Colour": [COLOUR_SEQUENCE[0], COLOUR_SEQUENCE[1], COLOUR_SEQUENCE[1], COLOUR_SEQUENCE[1]],
    })
    fig = px.bar(
        metric_df,
        x="Metric",
        ...
        color="Metric",
        text="Score",
        color_discrete_map={metric: colour for metric, colour in zip(metric_df["Metric"], metric_df["Colour"])},
        title=title,
    )
...
def main():
    ...
    if current_page == "Model Explorer":
        ...
        if not st.session_state.get("model_trained", False) or st.session_state.get("dataset_name") != dataset_name:
            ...
            st.sidebar.info("Train a model to see the SuperTree visualisation and evaluation metrics. Use the Predictions page after training to test a new transaction.")
            return
        ...
        metrics_col, confusion_col = st.columns(2)
        with metrics_col:
