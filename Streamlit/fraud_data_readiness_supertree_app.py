...
def show_stacked_histograms(df: pd.DataFrame):
    plot_df = get_display_dataframe(df)
    melted = plot_df.melt(
...
def show_data_visualisation(df: pd.DataFrame):
    ...
    vis_type = st.radio(
        ...
        ("Histograms", "Scatter Matrix", "Correlation Heatmap", "Class Balance"),
        horizontal=True,
    )
...
def main():
    ...
    if st.sidebar.button("Make Prediction", key="predict_obs"):
        ...
        fig = px.bar(
            ...
            orientation="h",
            color="Class",
            color_discrete_map={CLASS_NAMES[0]: COLOUR_SEQUENCE[0], CLASS_NAMES[1]: COLOUR_SEQUENCE[2]},
        )
        fig.update_layout(
