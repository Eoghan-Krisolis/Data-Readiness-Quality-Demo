...
DATASET_DESCRIPTIONS = {
    "Dataset A - Poor Data Quality": "This dataset contains duplicates, missing values, inconsistent category labels and unrealistic values. The tree is trained on the raw dataset with only minimal technical preprocessing so those issues still affect the model.",
...
def show_feature_distributions(df: pd.DataFrame):
    ...
    st.markdown("Choose how to view the feature distributions.")
    breakdown_by_target = st.toggle(
        "Break down plots by target variable",
        value=True,
        help="When enabled, each plot is coloured by whether the transaction is legitimate or fraudulent.",
    )
    ...
    if feature_type == "Numeric Features":
        ...
        melted = plot_df.melt(
            id_vars=[TARGET_COLUMN, "target_name"],
        ...
        melted["Value"] = melted["Value"].fillna("Missing").astype(str)
        ...
        bar_args = dict(
            ...
            facet_col="Feature",
            facet_col_wrap=2,
            category_orders={"Value": sorted(melted["Value"].unique().tolist())},
        )
...
def show_data_visualisation(df: pd.DataFrame):
    ...
    vis_type = st.radio(
        ...
        ("Feature Distributions", "Scatter Matrix", "Correlation Heatmap", "Class Balance"),
        horizontal=True,
    )
...
def make_prediction_input(defaults: pd.Series) -> pd.DataFrame:
    st.sidebar.subheader("Make a Prediction for a New Transaction")
...
def main():
    ...
    st.markdown(
        "Use the controls in the sidebar to explore how data readiness changes the decision tree produced for transaction fraud detection."
    )
    st.header("🌳 Demonstration with Decision Trees")
    st.markdown(
        "This app demonstrates a **Decision Tree Classifier** on three transaction fraud datasets. Each dataset represents a different level of data readiness so learners can compare the model structure and the performance produced by the same algorithm.",
    ...
    dataset_name = st.sidebar.radio("Training Dataset", options=list(DATASET_FILES.keys()))
    #st.sidebar.caption(DATASET_DESCRIPTIONS[dataset_name])

    st.sidebar.subheader("Set maximum tree depth")
    max_depth = st.sidebar.slider("Max Depth", min_value=1, max_value=8, value=4, step=1)
    ...
    if dataset_name == "Dataset A - Poor Data Quality":
        st.warning(
            "To keep the dataset genuinely dirty, duplicates and inconsistent category labels are left in place. Only minimal preprocessing is applied so the decision tree can technically train."
        )
    ...
    if not st.session_state.get("model_trained", False) or st.session_state.get("dataset_name") != dataset_name:
        ...
        st.subheader("Comparison Across the Three Datasets")
        st.dataframe(
        ...
        st.sidebar.info("Train a model to see the SuperTree visualisation, evaluation metrics and a single-transaction prediction.")
        return

    st.subheader(f"Trained {max_depth}-Level Decision Tree")
    st.components.v1.html(st.session_state["viz_html"], height=760)

    train_predictions = st.session_state["trained_pipeline"].predict(selected_df[FEATURE_COLUMNS])
    train_metrics = compute_metrics(selected_df[TARGET_COLUMN].astype(int), train_predictions)
    display_metrics(train_metrics, "Training Set Metrics")
    ...
    display_metrics(test_metrics, "Balanced Test Set Metrics")
    plot_confusion(test_df[TARGET_COLUMN].astype(int), test_predictions, "Balanced Test Set Confusion Matrix")
    ...
    if st.sidebar.button("Make Prediction", key="predict_obs"):
        ...
        st.subheader(
            f"🔮 Prediction Results - Predicted Class: {CLASS_NAMES[prediction]}"
        )
